...
1. 以源 PCAP/packet CSV 为不可拆分 `capture_group`，按最终模型样本数迭代优化 `80%/10%/10%` 划分；
...
本版本特征 ID 固定为 `segment15_burstp95_v1_2`。每轮只用候选训练组计算 `D_max`，再按容量规则统计各采集组的最终模型样本数，最多迭代3轮。full总体比例必须位于80/10/10各自±3个百分点内，每个应用的验证和测试占比均不得低于5%；失败时保留诊断但不写成功标记。验证集和测试集不做重采样。
